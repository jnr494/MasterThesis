# -*- coding: utf-8 -*-
"""
Created on Fri Apr 16 22:36:28 2021

@author: https://github.com/LinusBleistein/Signature-inversion/blob/master/insertion.py
"""
...
if __name__ == '__main__':
    ...
    path = np.random.normal(size = 10)*0.25
    path[0] = 0
    path = np.cumsum(path)
    ...
    p_path = torch.from_numpy(ll_path[np.newaxis,...])
    
    order = 8
    
    signature = signatory.signature(p_path, order)
    inverted_path = invert_signature(signature, order, 2)[0]
    #signature2 = signatory.signature(inverted_path, order)

    plt.plot(ll_path[:,0],ll_path[:,1])
    plt.plot(inverted_path[:,0],inverted_path[:,1])
    plt.show()