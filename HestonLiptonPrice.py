...
def HestonProb(x, tau, rho, sigma, kappa, theta, r, nu, K, type_):
    ...
    integral = integrate.quad(int_func, 0.0001, 5000)
...
def heston_lipton_callprice_vec(spot, timetoexp, strike, r, divyield, V, theta, kappa, epsilon, rho, greek=1):
    #print(spot, timetoexp, strike, r, divyield, V, theta, kappa, epsilon, rho, greek)
    X = np.log(spot / strike) + (r - divyield) * timetoexp
    kappahat = kappa - 0.5 * rho * epsilon
    xiDummy = kappahat**2 + 0.25 * epsilon**2
    ...
    dummy = integrate.quad_vec(integrand, -100, 100)[0]
...
if __name__ == "__main__":
    ...
    spots = np.random.uniform(70,130,size = 1000)
    vs = np.random.uniform(low = 0.05,high = 0.15,size = 1000)
    
    t1 = time.time()
    greeks1 = heston_lipton_callprice_vec(spot = spots, timetoexp = 1, strike = 100, r = 0.03, divyield =0., 
                                         V = vs, theta = 0.09, kappa = 2, epsilon = 0.3, rho = -0.9, greek = 2)
    t2 = time.time()
    print(t2-t1)
    greeks2 = [heston_lipton_callprice(spot = s, timetoexp = 1, strike = 100, r = 0.03, divyield =0., 
                             V = v, theta = 0.09, kappa = 2, epsilon = 0.3, rho = -0.9, greek = 2) for s,v in zip(spots,vs)]
    t3 = time.time()
    print(t3-t2)