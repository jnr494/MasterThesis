...
def create_MG(N, alpha = 0.9, beta = 10, seed = None, plot = True, overlap = False):
    MG = MarketGenerator.MarketGenerator(s_model, n)
    MG.create_vae(latent_dim = n, layers_units=[40], alpha = alpha, beta = beta, gamma = 0, real_decoder = True)
    if overlap is False:
        MG.create_training_path(N, overlap = False, seed = seed, cheat = True)
    else:
...
N_MG_samples = 50 #int(n*(1/T)*2 - n + 1) # 5 years of data with n samples in T time.
overlap = False
n_MG_models = 10 
...
for i in range(n_MG_models):
    ...
    if (i == 0) or (use_same_MG is False):
        MGs.append(create_MG(N_MG_samples, beta = betas[i], seed = train_path_seed, overlap = overlap))
    else:
       MGs.append(MGs[0]) 
...
for i in range(2):
    for j in range(n_assets):
        ...
        plt.savefig("MGexp1_{}_hs_sample_{}_{}.eps".format(exp_nr,i,N_MG_samples), bbox_inches='tight')
        plt.show()
        plt.close()
...
if save_output is True:
    text_file = open(folder_name + "output MG exp 1_{}, hedge points {}, tc = {}, samples = 2_{}, n MG samples = {}, same MG = {}, seed = {}, final act = {}, overlap = {}.txt".format(exp_nr,n,tc,N, N_MG_samples, use_same_MG, tmp_seed_bool, fi_ac_str, overlap),"w")
