...
option_payoffs = option_payoff(S1)
print(np.exp(-r*T)*np.mean(option_payoffs))

option_price = BScallprice(s0 = S0, sigma = sigma, r = r, T = T, K = strike)
...
delta = BScallprice(s0 = S0, sigma = sigma, r = r, T = T, K = strike, greek = "delta")
print("Delta:",delta)

def get_analytical_h(T):
    p0 = BScallprice(s0 = S0, sigma = sigma, r = r, T = T, K = strike)
    S1 = get_S1(T)
    option_payoffs = option_payoff(S1)
...
deltas = [BScallprice(s0 = S0, sigma = sigma, r = r, T = T_, K = strike, greek = "delta") for T_ in Ts]
