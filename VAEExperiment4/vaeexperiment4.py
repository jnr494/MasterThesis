...
init_var = np.quantile(org_cond, 0.9)
MG.cond_n = 0
...
plt.ylim((0.75,1.35))
plt.xlabel("t")
plt.savefig("vae4_example_paths_MG_{}_{}.eps".format(cheat, beta), bbox_inches='tight')
plt.show()

plt.plot(pc.times, training_paths[:no_of_plotted_paths,:].T)
plt.ylim((0.75,1.35))
plt.xlabel("t")
plt.savefig("vae4_example_paths_Heston_{}_{}.eps".format(cheat, beta), bbox_inches='tight')
plt.show()
...
plt.legend()
plt.xlabel("t")
plt.savefig("vae4_means_comp_{}_{}.eps".format(cheat, beta), bbox_inches = "tight")
plt.show()  
...
plt.legend()
plt.xlabel("t")
plt.savefig("vae4_stds_comp_{}_{}.eps".format(cheat, beta), bbox_inches = "tight")
plt.show() 
...
plt.savefig("vae4_corrs_comp_{}_{}.eps".format(cheat, beta), bbox_inches = "tight")
plt.show() 
...
plt.xlabel("Lag")
plt.ylim((c_min,c_max))
plt.savefig("vae4_abs_corrs_comp_{}_{}.eps".format(cheat, beta), bbox_inches = "tight")
plt.show() 
...
plt.savefig("vae4_qq_{}_{}.eps".format(cheat, beta), bbox_inches = "tight")
plt.show()
...
plt.legend()
plt.xlabel("spot")
plt.savefig("vae4_ecdf_T_{}_{}.eps".format(cheat, beta), bbox_inches = "tight")
plt.show()
...
plt.ylim((0,1))
plt.xlabel("t")
plt.savefig("vae4_ks_pvalues_{}_{}.eps".format(cheat, beta), bbox_inches = "tight")
plt.show()
...
for n_samples, overlap in zip(n_sampless, overlaps):
    tmp_pcs = [[] for q in quantiles]
    
    for i in tqdm(range(n_vaes)):
        print("alpha, beta, gamma, samples, overlap:",alpha,beta, gamma, n_samples, overlap)
        #Creat MG and train
        ...
        tmp_MG.create_training_path(n_samples, overlap = overlap, seed = None, cheat = False)
...
for q_idx, q in enumerate(quantiles):
    for i, pcs in enumerate(pcss):
        ...
        plt.plot(pc.times[1:], pvalues_mean, label = r"VAE, {}".format(names[i]), c = 'C{}'.format(i+1))
    ...
    #plt.savefig("vae4_ks_avg_{}_q{}.eps".format(var_name2, int(q*100)), bbox_inches = "tight" )
    plt.show()
...
for i, (m, s) in enumerate(zip(ks_means,ks_stds)):
    plt.plot(quantiles, m, label = r"VAE, {}".format(names[i]), c = 'C{}'.format(i+1))
...
plt.ylim((0,1))
plt.xlabel(r"$\nu(0)$-quantile")
plt.savefig("vae4_ks_avg_avgq.eps", bbox_inches = "tight" )
plt.show()
...
for i, pcs in enumerate(pcss):
    ...
    plt.plot(pc.lags, mg_corrs, c = 'C{}'.format(i+1), label = r"VAE, {}".format(names[i]))
    #plt.plot(np.tile(pc.lags,(2,1)).T, conf_interval.T ,'--', c = 'C{}'.format(i+1))
plt.ylim((c_min,c_max))
plt.xlabel("Lag")
plt.title("Quantile:{}".format(q))
plt.legend()
plt.savefig("vae4_corr_avg_q{}.eps".format(q), bbox_inches = "tight" )
plt.show()
...
for i, pcs in enumerate(pcss):
    ...
    plt.plot(pc.lags, mg_abs_corrs, c = 'C{}'.format(i+1), label = r"VAE, {}".format(names[i]))
    #plt.plot(np.tile(pc.lags,(2,1)).T, conf_interval.T ,'--', c = 'C{}'.format(i+1))
plt.ylim((c_min,c_max))
plt.xlabel("Lag")
plt.legend()
plt.title("Quantile:{}".format(q))
plt.savefig("vae4_abscorr_avg_q{}.eps".format(q), bbox_inches = "tight" )
plt.show()
